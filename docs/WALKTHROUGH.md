...
22. **The lowRISC toolchain "doesn't run" on Windows** → its tar.xz contains Linux ELF binaries; extracted natively on Windows they simply cannot execute. Install it inside WSL (FREERTOS_PORT.md, Toolchain) — the locators find it there and build.bat compiles through `wsl` automatically.
