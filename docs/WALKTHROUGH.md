...
30. **Verilator flow quirks (2026-08-19, all encoded in `ibex_soc.sh`)** → (a) Verilator resolves module refs even inside dead generate blocks: the disabled dummy-instr block references `prim_lfsr`, which Verilator finds via the incdirs and which then needs `prim_cipher_pkg` — fed explicitly (`VLT_EXTRA`), NOT added to filelist.f (xsim/Vivado never need it). (b) On MSYS2/Windows hosts, GCC 16.2 at `-Os` (Verilator's default OPT) drops std::string's move-ctor instantiation → undefined reference at link; the script forces `-O2` there (Linux keeps defaults). (c) MSYS2's *msys*-flavored python mangles `C:/` paths in Verilator's `verilator_includer` step — install the *ucrt64* python (`mingw-w64-ucrt-x86_64-python`) so the native one wins on PATH.
