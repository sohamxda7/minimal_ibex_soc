...
| `sw/freertos/build.sh` | The POSIX twin of build.bat: same compile line for Linux/WSL (how the Linux-only lowRISC toolchain builds on Windows) | `./build.sh [hw\|sim] [toolchain-root]` from `sw/freertos` (build.bat calls it via `wsl` automatically) | Keep its compile line in sync with build.bat. LF-only (`.gitattributes` enforces) |
