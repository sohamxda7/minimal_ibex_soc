...
| Teammate / field robustness (2026-08-19 → 20) | Four field reports closed at the root: scripts committed non-executable (every script now invoked as `bash <script>`, plus `--chmod=+x`), "it asks for my GCC path every run" (a swallowed *Permission denied* from that same cause), `Illegal instruction 0xffffffff` from tb_freertos (a missing program image — benches now verify their image up front), and twelve `No such file or directory` from gcc (`build.sh` is now cwd-independent, both build scripts verify every source first, and `setup` on both OSes opens with a `git ls-files --deleted` integrity line). **Tool profiles** (`sim`/`fpga`/`full`/`auto`) let a PC run Verilator without Vivado or Vivado without Verilator — the unused tool is `[SKIP]`, never `[FAIL]`, never prompted for; Windows can now run the whole 11-sim Verilator regression through MSYS2 from the GUI |
