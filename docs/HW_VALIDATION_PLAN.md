...
| 15 | I2C bus scan | boot line `toy: i2c scan: 3C 76` then `toy: lcd up, oled=0 bme=0` — both parts ACK (or `toy: oled attached` / `toy: bme attached` if wired after boot: parts are re-probed every 5 s) | **OLED half PASSED 2026-08-20** (after rewiring off the breadboard edge rails): scan reports `3C` only. Sensor ACKs at no address on a bus proven by the working OLED, with power/CSB/SDO/continuity all verified at its pins → **module dead, WIP pending replacement** (report §12b). Re-soldered and retested 2026-08-20: unchanged, and an on-demand `i` re-scan confirms it live |
| 16 | BME280 | plausible signed T + P + H: `T=...cC P=...Pa H=...m%` on UART every 10 s (with `t` reports on) and `T= 25.34C H= 45.6%` on the LCD live block | ☐ **WIP — replacement module on order** (the first one is dead, §12b); firmware path is sim-proven and unchanged |
| 17 | SSD1306 | full status screen: double-height ARF logo, identity lines, rule, live uptime+spinner / pattern+speed+key / rgb+heartbeat, sensor row, sweeping activity bar | **PASS 2026-08-20** — confirmed visually on the bench: panel renders and updates every second, independent of the sensor (the sensor row reads `bme280: not fitted` until a working part arrives) |
